...
const themes = [
    ...
    "Контраст. Цвет, свет, масштаб, смысл, ...",
    "Минимализм (много воздуха)",
    "Фокус на текстуру",
    "Нижний ракурс. Съёмка снизу вверх",
    "Верхний ракурс. Съёмка сверху вниз",
    "Визуальная рифма",
    "Многослойность"
];
...
/* Оптимизированный воркер таймера */
const workerCode = `
    let timer = null;
...
/* Обновление интерфейса слайдеров */
function updateUI() {
...
/* Логика клика на главную кнопку */
btn.onclick = () => {
    if (state === 'idle') {
...
function triggerShot() {
    ...
    btn.className = "state-shot"; // Переключаем на фазу кадра
...
function nextPhase() {
    state = 'walk';
    playSignal('walk');
    btn.className = "state-walk"; // Возвращаем фазу ходьбы
    document.getElementById('btnText').textContent = "СТОП";
    cycle();
}

/* Безопасный инжект отслеживания состояния СТОП */
const btnTextNode = document.getElementById('btnText');
const stopInstr = document.getElementById('stopInstruction');
if (btnTextNode && stopInstr) {
    const observer = new MutationObserver(() => {
        if (btnTextNode.innerText.includes('СТОП')) {
            stopInstr.style.visibility = 'visible';
            stopInstr.style.opacity = '1';
        } else {
            stopInstr.style.visibility = 'hidden';
            stopInstr.style.opacity = '0';
        }
    });
    observer.observe(btnTextNode, { childList: true, characterData: true, subtree: true });
}
